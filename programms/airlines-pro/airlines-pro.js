...
let confirmForWhile;
const flightsIsFull =
  "All flight are created, please delete a flight to add a new one.";
const flightsIsClear =
  "This id is erased, please create a new one before delete.";
...
const showAllFlights = (arr) => {
  ...
  arr.forEach((flight) => {
    if (flight.scale === true) {
      console.log(
        `The flight ${flight.id} from ${flight.from} to ${flight.to} have a price of ${flight.cost}€ and have a scale.`
      );
    } else {
      console.log(
        `The flight ${flight.id} from ${flight.from} to ${flight.to} have a price of ${flight.cost}€ is direct.`
      );
    }
  });
};
...
const createFlight = () => {
  if (flights.length === 15) {
    ...
    do {
      ...
      confirmForWhile = prompt("Create anyone else? (yes/no)");
      if (confirmForWhile === "yes") {
        confirmForWhile = true;
      }
    } while (flights.length <= 14 && !confirmForWhile);
  }
};
...
const actionsAdmin = () => {
  ...
  do {
    action = prompt(
      "Welcome admin, what do you want to do? (c to create a new flight or d to delete a flight)"
    );
    if (action !== null) {
      action.toLowerCase();
    }
...
const actionsUser = () => {
  ...
  const flightsByCost = [];
  const flightBought = [];
  const priceToBuy = prompt("How much do you want to spend?");
  ...
  if (flightBought.scale === true) {
    console.log(
      `The flight ${flightBought.id} from ${flightBought.from} to ${flightBought.to} have a price of ${flightBought.cost}€ and have a scale.`
    );
  } else {
    console.log(
      `The flight ${flightBought.id} from ${flightBought.from} to ${flightBought.to} have a price of ${flightBought.cost}€ is direct.`
    );
  }
};
